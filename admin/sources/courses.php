<?php 
	...
	switch ($act) {
        case 'man':
			get_items();
			$templates = 'courses/items';
			break;
		case 'add':
			$templates = 'courses/item_add';
			break;
		case 'edit':
			get_item();
			$templates = 'courses/item_add';
	...
	function get_items(){
		...
		if($config['paging-table']==true){
		    $items = $d->rawQuery("SELECT * from #_courses where type=? order by id desc",$param);
		}else{
			$per_page = 10;
	        $startpoint = ($page * $per_page) - $per_page;
	        $limit = ' limit '.$startpoint.','.$per_page;
	        $sql = "SELECT * from #_courses where type=? order by id desc ".$limit;
	        $items = $d->rawQuery($sql,$param);
	        $sqlq = "SELECT COUNT(*) as `num` from #_courses where type=? order by id desc";
	        $count = $d->rawQueryOne($sqlq,$param);
	       	$total = $count['num'];
	        $url = 'index.html?com=courses&act=man'.$url_type;
	...
	function get_item(){
		global $d,$item;
		$id = (int)$_GET['id'];
		$item  =  $d->rawQueryOne("select * from #_courses where id=?",array($id));
		if(empty($item)){
	...
	function save_item(){
    	...
    	if($id){
			...
			if ($d->update('courses', $data)) {
				...
				$func->redirect('index.html?com=courses&act=edit'.$url_type.'&id='.$id.'&message='.$message);
			} else {
			  	$func->transfer('Không nhận được dữ liệu','index.html?com=courses&act=man'.$url_type);
			}
    	}else{
			...
			if ($id_insert) {
				...
				$message = base64_encode(json_encode($result));
				$func->redirect('index.html?com=courses&act=man'.$url_type.'&message='.$message);
			}
    	}
	}
	function delete_item(){
		...
		if(isset($_GET['id'])){
			$item  =  $d->rawQueryOne("select id from #_courses where id=?",array($id));
			if($item){
				$d->where('id', $item['id']);
				$d->delete('courses');
	        	$result['status'] = 200;
			...
			if(count($listid)){
				...
				$func->transfer('Không nhận được dữ liệu','index.html?com=courses&act=man');
			}
		}
