<?php 
    ...
    $courses_lists = $d->rawQuery("SELECT * from #_courses where type=? and view=? order by id desc",array("dang-ky-khoa-hoc",0));
...
<nav class="navbar-custom-menu navbar navbar-expand-lg m-0 <?=($_GET['com']=='orders' && $_GET['act']=='add') ? 'active':''?>">
    ...
    <div class="d-flex flex-grow-1">
        <ul class="navbar-nav flex-row align-items-center ml-auto">
            ...
            <li class="nav-item dropdown notification">
                <a class="nav-link dropdown-toggle <?php if(count($courses_lists)>0){ ?>badge-dot<?php } ?>" href="#" data-toggle="dropdown">
                    <i class="typcn typcn-folder-add"></i>
                </a>
                <div class="dropdown-menu dropdown-menu-right">
                    <h6 class="notification-title">Thông tin đăng ký khóa học</h6>
                    <p class="notification-text">Bạn có <?=count($courses_lists)?> đăng ký chưa đọc</p>
                    <div class="notification-list">
                        <?php if(count($courses_lists)>0){ foreach ($courses_lists as $k => $v) { ?>
                        <div class="media new">
                            <div class="media-body" onclick="window.location.href='index.html?com=courses&act=edit&type=dang-ky-khoa-hoc&id=<?=$v['id']?>'">
                                <h6><?=$v['subject']?></h6>
                                <span><?=$v['createdAt']?></span>
                            </div>
                        </div><!--/.media -->
                    ...
                    <div class="dropdown-footer"><a href="index.html?com=courses&act=man&type=dang-ky-khoa-hoc">Xem tất cả đăng ký</a></div>
                </div><!--/.dropdown-menu -->
            </li><!--/.dropdown-->
