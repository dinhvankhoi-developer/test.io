<?php 
    $csetting = $setting['courses'][$_GET['type']];
?>
<div class="content-header row align-items-center m-0">
    <nav aria-label="breadcrumb" class="col-sm-4 order-sm-last mb-3 mb-sm-0 p-0 ">
        <ol class="breadcrumb d-inline-flex font-weight-600 fs-13 bg-white mb-0 float-sm-right">
            <li class="breadcrumb-item"><a href="index.html">Trang chủ</a></li>
            <li class="breadcrumb-item"><a href="index.html?com=courses&act=man<?=$url_type?>">Danh sách</a></li>
            ...
            <div class="card mb-4">
                <div class="card-header">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div>
                            <a class="btn btn-danger w-100p ml-1 deleteChoose" href="index.html?com=courses&act=delete<?=$url_type?>" role="button">Xóa chọn</a>
                            <!-- <a class="btn btn-success w-100p ml-1" href="index.html?com=courses&act=add<?=$url_type?>" role="button">Thêm mới</a> -->
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table <?php if($config['paging-table']==true){ ?>v table-borderless no-styling basic<?php }else{ ?>display table-striped table-hover table-border<?php } ?>">
                            ...
                            <tbody>
                                ...
                                <tr <?php if($v['view']==0){ ?>style="background: #f5f5f5; font-weight: bold;"<?php } ?>>
                                    <td>
                                        <div class="check-table<?=($config['paging-table']==true) ? '':' auto'?>">
                                    ...
                                    <td><?=$v['name']?></td>
                                    <td>
                                        <?= $func->getInfoItems('khoa-hoc',$v['id_product'],'lists','name_vi')?>
                                    </td>
                                    <td><?=$v['price']?></td>
                                    ...
                                    <?php } ?>
                                    <td>
                                        <a href="index.html?com=courses&act=edit<?=$url_type?>&id=<?=$v['id']?>" class="btn btn-success-soft btn-sm mr-1"><i class="typcn typcn-eye-outline"></i></a>
                                        <a href="index.html?com=courses&act=delete<?=$url_type?>&id=<?=$v['id']?>" onClick="if(!confirm('Xác nhận xóa')) return false;" class="btn btn-danger-soft btn-sm"><i class="typcn typcn-trash"></i></a>
                                    </td>
                                </tr>
                                <?php } } ?>
                            </tbody>
