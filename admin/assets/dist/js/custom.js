...
$(document).ready(function() {
    ...
    $('.deleteImgMulti').click(function(event) {
        ...
        $('body').on('click', '.delete-kh', function() {
            var value = $(this).attr("value");
            $.ajax(
            {
                url:"ajax/ajax_xoaBook.php",
                type: 'post',
                data:{
                    id: value
                },
    ...
    $('body').on('click', '.addBook', function() {
        var value = $(".my_select_box").val();
        var id = $(this).data('id');
        $.ajax(
        {
            url:"ajax/ajax_addBook.php",
            type: 'post',
            data:{
                id_product: value,
                id_user: id,
            ...
            success: function (result){
                ...
                {
                    alert("Đã được thêm trước đó.");
                }
                else if(result == 3)
                {
                    alert("Không đủ điểm. Nạp thêm điểm.");
                }
            }
        });
    });
