...
<body <?php if($source!='index'){ ?>class="in-page"<?php } ?>>
	...
	<?php }else{ ?>
		...
		<?php if($source=='index'){ ?>
			<?php require_once _layouts.'slider.php'; ?>
		<?php } ?>
	...
	<script type="text/javascript">
		var _SESSION_Login_user = '<?=$_SESSION['signin']?>';
		if (_SESSION_Login_user =='') {
		  const fpPromise = import('https://fpjscdn.net/v3/G0H4VXxjHQX40fIshibx')
		...
		function checkSessLogin() {
			const url = 'check_session.php?time=' + Math.random();
			$.ajax({url, dataType: "json", success: function(res) {
				if(!res.isLogin) {
					alert(res.message);
					location.reload();
					setTimeout(() => {
					alert('Tài khoản của bạn đã đăng nhập quá 4 thiết bị!')
					}, 2000);
		...
		if (_SESSION_Login_user) {
			setInterval(()=>{checkSessLogin()}, 2000)
		}
