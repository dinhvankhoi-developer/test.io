		...
		if(!empty($_POST)){
			...
			if($error['status'] == 200){
				...
				if(!empty($row_user)){
					...
					$items_sss = $d->rawQueryOne("select lastsession_id from #_customers where id = ?  limit 0 ,1",array($_SESSION['signin']['id']));
					$ex_MACC_id = explode(',',$items_sss['lastsession_id']);
					$id_session_ro = $_SESSION['MACC'];
					
					if (count($ex_MACC_id) < 4 && !in_array($id_session_ro,$ex_MACC_id)) {
					   	$product_customer_login_ss = array();
					    //$id_session_ro = session_id();
					     
					    if(!empty($items_sss['lastsession_id'])){
					     $product_customer_login_ss = explode(",",$items_sss['lastsession_id']);
					    }
					    array_push($product_customer_login_ss,$id_session_ro);
					    $product_login_ss_update =implode(",",$product_customer_login_ss);
					    $updateItems = $d->rawQueryOne("update #_customers set lastsession_id = '$product_login_ss_update'  where id = ?  ",array($_SESSION['signin']['id']));

					}else{
						//var_dump($ex_session_id_check);
						if($items_sss && in_array($id_session_ro,$ex_MACC_id)) {
						 	//var_dump(in_array($id_session_ro,$ex_session_id_check));
						}else{
							//var_dump($id_session_ro);
							//$notice_adminw = 'Có người đang đăng nhập tài khoản của bạn! Tài khoản của bạn đã đăng nhập quá 4 thiết bị!';

							session_destroy();
							$func->transfer("Tài khoản của bạn đã đăng nhập quá 4 thiết bị!", $config_base);
							$status = false;
						}
					}
