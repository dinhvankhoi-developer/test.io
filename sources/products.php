<?php
	...
	if(empty($id) && empty($idl) && empty($idc) && empty($idi) && empty($ids) ){
		...
		$field_load .= ", price, price_old, id_list,id_cat,id_item,id_sub,tags,code,status, desc_$lang as descs, content_$lang as content,createdAt,trademark,specification,updatedAt,parameter_$lang as parameter,view";
		...
		if ($type == 'khoa-hoc') {
			if(empty($_SESSION['signin'])){
				$func->transfer("Bạn cần đăng nhập để xem được nội dung này!", $config_base.'account/dang-nhap');
			}
			$_SESSION['id_product_course'] = $row_detail['id_list'];
			$row_course = $d->rawQueryOne("select id,name, id_product,price from #_courses where type=? and author_id=? and id_product=? and find_in_set ('hienthi',status)",array("dang-ky-khoa-hoc",$_SESSION['signin']['id'],$row_detail['id_list']));

			$row_course_non = $d->rawQueryOne("select id,name, id_product,price from #_courses where type=? and author_id=? and id_product=?",array("dang-ky-khoa-hoc",$_SESSION['signin']['id'],$row_detail['id_list']));
			if ($row_course) {
			}else{
				if ($row_course_non) {
					...
					$func->transfer("Bạn cần đăng ký với admin hoặc mua khóa học này mới xem được nội dung!", $config_base.'dang-ky-khoa-hoc');
				}
			}
